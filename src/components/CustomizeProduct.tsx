...
const CustomizeProduct = ({
    ...
    const handleOptionSelected = (optionsType: string, choice: string) => {
        setSelectedOptions((prev) => ({ ...prev, [optionsType]: choice }))
    }
                    ...
                    <ul className='flex items-center gap-3'>
                        {
                            option.choices?.map((choice) => {
                                ...
                                return option.name === "Color" ?
                                    (
                                        <li className='w-8 h-8 rounded-full ring-1 ring-gray-300 relative' style={{ backgroundColor: choice.value, cursor: disabled ? "not-allowed" : "pointer" }}
                                        ...
                                        >
                                            ...
                                            {disabled && <div className='absolute w-10 h-[2px] bg-red-400 rounded-full rotate-45 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ' />}
                                        </li>
                                    )
                                    : (
                                        <li className='ring-1 ring-lama rounded-md py-1 px-4 text-sm'
                                            style={{
            ...
            {/* <h4 className='font-medium'>Choose a {option.name}</h4>
                    <ul className='flex items-center gap-3'>
                        ...
                        <li className='w-8 h-8 rounded-full ring-1 ring-gray-300 cursor-not-allowed relative bg-green-500'>
                            <div className='absolute w-10 h-[2px] bg-red-400 rounded-full rotate-45 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ' />
                        </li>
                    </ul> */}
